...
const ServiceMatcher: React.FC = () => {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      if (svgElement) {
        ...
        // Ensure standard XML namespaces are present
        clonedSvg.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
        clonedSvg.setAttribute('xmlns:xlink', 'http://www.w3.org/1999/xlink');
        
        // Capture relevant styles from the page to ensure colors are preserved
        ...
        const scale = 2; // High DPI for the document
        const rect = svgElement.getBBox(); // Get actual content bounds
        ...
        await new Promise((resolve, reject) => {
          img.onload = () => {
            if (ctx) {
              // Draw white background so it looks good in Word/PDF
      ...
      console.error("Critical failure during diagram rasterization:", err);
    }
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>AWS Architectural Design Record</title>
        <!--[if gte mso 9]>
        <xml>
          <w:WordDocument>
            <w:View>Print</w:View>
        ...
        <style>
          ...
          .section-title { font-size: 14pt; font-weight: bold; color: #2563eb; margin-top: 0; margin-bottom: 15px; text-transform: uppercase; letter-spacing: 1px; }
          ...
          .diagram-wrap { background: #1e293b; border-radius: 8px; padding: 20px; text-align: center; margin: 20px 0; }
          .diagram-img { width: 100%; height: auto; display: block; }
          .step-phase { font-weight: bold; color: #4338ca; }
          .footer { font-size: 8pt; color: #94a3b8; text-align: center; margin-top: 60px; border-top: 1px solid #f1f5f9; padding-top: 20px; }
        </style>
      </head>
      <body>
        <div class="header">
          <h1 class="title">Architectural Decision Record</h1>
          <div class="meta">AWS Solutions Architect Board Review | Date: ${dateStr}</div>
        </div>

        <div class="section">
          <h2 class="section-title">I. Principal Service Recommendation</h2>
          <div class="hero-recommendation">${result.recommendedService}</div>
          <p><strong>Justification:</strong> ${result.justification}</p>
        </div>

        <div class="section">
          <h2 class="section-title">II. High-Level System Topology</h2>
          <div class="diagram-wrap">
            ${diagramImageUrl ? `<img src="${diagramImageUrl}" class="diagram-img" alt="AWS Infrastructure Flow" />` : '<p style="color:white;">[Infrastructure Diagram Snapshot]</p>'}
          </div>
          <p style="font-size: 8pt; color: #64748b; font-family: monospace;">Logic Definition: ${result.mermaidDiagram}</p>
        </div>

        <div class="section">
          <h2 class="section-title">III. Detailed Implementation Roadmap</h2>
          ${result.implementationSteps.map((s, i) => `
            <div style="margin-bottom: 15px; border-bottom: 1px solid #f1f5f9; padding-bottom: 10px;">
              ...
              <span style="font-size: 10pt; color: #475569;">${s.details}</span>
            </div>
          `).join('')}
        </div>

        <div class="section">
          <h2 class="section-title">IV. Alternatives & Risk Assessment</h2>
          <ul>
            ${result.alternatives?.map(alt => `<li>${alt}</li>`).join('') || '<li>No significant alternatives identified.</li>'}
          </ul>
        </div>

        <div class="footer">
          Generated via AWS Architect Masterclass AI Engine | Confidential Board Briefing
        </div>
      </body>
      </html>
    `;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (error) { alert("Architecture design failed. Check connectivity."); } finally { setLoading(false); }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-12 animate-fadeIn pb-24">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="flex p-1.5 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md">
          <button onClick={() => { setMode('conversational'); setResult(null); }} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all ${mode === 'conversational' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
            <ChatBubbleBottomCenterTextIcon className="w-5 h-5" /> Quick Pitch
          </button>
          <button onClick={() => { setMode('structured'); setResult(null); }} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all ${mode === 'structured' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
            <QueueListIcon className="w-5 h-5" /> Architect's Brief
          </button>
          <button onClick={() => { setMode('history'); setResult(null); }} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all ${mode === 'history' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
        ...
        <div className="space-y-12 animate-fadeIn">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-3 px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-lg disabled:opacity-50"
            >
              {isExporting ? <ArrowPathIcon className="w-5 h-5 animate-spin" /> : <DocumentArrowDownIcon className="w-5 h-5" />}
              {isExporting ? "Capturing Visuals..." : "Export Board Briefing (.doc)"}
            </button>
          </div>
          ...
          <div className="glass p-12 rounded-[4rem] border border-emerald-500/20 bg-emerald-500/[0.02] relative overflow-hidden">
            <div className="flex flex-col lg:flex-row gap-16 relative z-10">
              <div className="flex-1 space-y-10">
                <div>
                  <h4 className="text-xs font-black text-emerald-400 uppercase tracking-[0.4em] mb-6">Master Recommendation</h4>
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-black text-blue-400 uppercase tracking-[0.4em]">Architectural Justification</h4>
                  <p className="text-slate-300 leading-relaxed text-xl font-light italic border-l-[6px] border-blue-500/30 pl-8 py-2">{result.justification}</p>
                </div>
              </div>
