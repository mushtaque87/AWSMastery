            ...
            <div className="glass p-12 rounded-[4rem] border border-blue-500/20 bg-blue-500/[0.01]">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">Infrastructure as Code (IaC)</h3>
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Enterprise Automation Blueprints</p>
                  </div>
                </div>
                ...
                <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/10 backdrop-blur-md">
                  <button 
                    ...
                    className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeIacTab === 'terraform' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
                  >
                    Terraform
                  </button>
                  <button 
                    onClick={() => setActiveIacTab('cloudformation')}
                    className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeIacTab === 'cloudformation' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
                  >
                    CloudFormation
                  </button>
                </div>
              ...
              <div className="h-[500px] overflow-hidden rounded-[2.5rem] border border-white/5">
                <IaCViewer 
                  code={activeIacTab === 'terraform' ? result.terraformCode || '' : result.cloudFormationCode || ''} 
                />
              </div>
              
              <div className="mt-8 flex items-center gap-3 px-6 py-4 bg-blue-500/5 rounded-2xl border border-blue-500/10">
                <CommandLineIcon className="w-5 h-5 text-blue-400" />
                <p className="text-xs text-slate-400 leading-relaxed font-medium">
                  Professional Tip: Always run <code className="text-blue-300">terraform plan</code> or <code className="text-blue-300">aws cloudformation validate-template</code> before applying these assets to production environments.
                </p>
              </div>
            </div>
          </div>
